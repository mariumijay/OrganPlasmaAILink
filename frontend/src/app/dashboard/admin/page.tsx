...
export default function AdminDashboard() {
  ...
  const handleBulkApprove = async () => {
    if (!confirm("Are you sure you want to approve ALL pending applications? This will grant live network access to all waiting entities.")) return;

    setIsDataLoading(true);
    try {
      const res = await fetch("/api/admin/bulk-approve", { method: "POST" });
      if (!res.ok) throw new Error("Bulk synchronization failed.");

      toast.success("Clinical Network Fully Verified!", {
        description: "All pending donors and hospitals are now LIVE."
      });
  ...
  const combinedDonors = [
    ...(bloodDonors || []).map((d: any) => ({ ...d, type: 'blood' })),
    ...(organDonors || []).map((d: any) => ({ ...d, type: 'organ' }))
        ...
        <div className="p-8 border-b border-border flex items-center justify-between bg-muted/30">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              disabled={isDataLoading || pendingApprovals.length === 0}
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-green-500/10 text-green-500 text-[10px] font-black uppercase tracking-widest hover:bg-green-500 hover:text-white transition-all border border-green-500/20 disabled:opacity-30 disabled:pointer-events-none"
            >
            ...
            <button
              ...
              className="px-6 py-2.5 rounded-xl bg-primary text-white text-[10px] font-black uppercase tracking-widest hover:brightness-110 transition-all shadow-lg shadow-primary/20"
            >
              Advanced View
            </button>
          </div>
              ...
              {isDataLoading ? (
                ...
                pendingApprovals.map((item) => (
                  <tr key={item.id} className="hover:bg-muted/5 transition-colors group">
                    ...
                    <td className="p-6">
                      <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-tighter border ${item.user_type === 'hospital' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'
                        }`}>
