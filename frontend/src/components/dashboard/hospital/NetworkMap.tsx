  ...
  return (
    <div className="relative h-[600px] w-full rounded-[2.5rem] overflow-hidden border border-border shadow-2xl glass-card">
      ...
      >
        ...
        {/* --- DONORS --- */}
        {donors.map((d, i) => {
          const lat = d.latitude || (d.city ? CITY_COORDS[d.city]?.[0] : null);
          const lng = d.longitude || (d.city ? CITY_COORDS[d.city]?.[1] : null);
          
          if (!lat || !lng) return null;
          
          // Offset slightly for multiple donors in same city if using city coords
          const finalPos: [number, number] = !d.latitude ? [
            lat + (Math.random() - 0.5) * 0.1,
            lng + (Math.random() - 0.5) * 0.1
          ] : [lat, lng];
          
          return (
            <Marker key={`donor-${d.id || i}`} position={finalPos} icon={donorIcon}>
              <Popup className="custom-popup">
                <div className="p-2">
                   ...
                   <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold text-muted-foreground">{d.city || 'Location Active'}</span>
                   </div>
                </div>
              </Popup>
            </Marker>
        ...
        {/* --- HOSPITALS --- */}
        {hospitals.map((h, i) => {
          const lat = h.latitude || (h.city ? CITY_COORDS[h.city]?.[0] : null);
          const lng = h.longitude || (h.city ? CITY_COORDS[h.city]?.[1] : null);
          
          if (!lat || !lng) return null;
          return (
            <Marker key={`hosp-${h.id || i}`} position={[lat, lng]} icon={hospitalIcon}>
              <Popup>
                <div className="p-2">
