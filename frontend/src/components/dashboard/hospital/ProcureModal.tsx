...
export function ProcureModal({ match, isOpen, urgency, onClose }: ProcureModalProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      // 1. Fetch current user session for hospital_id
      const { data: { user } } = await (await import("@/lib/supabase/client")).createClient().auth.getUser();
      
      const response = await fetch("/api/hospital/procure", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          donor_id: match.donor_id,
          hospital_id: user?.id,
          category: (match as any).donating_items?.length > 0 ? 'blood' : 'organ',
          organ_type: (match as any).donating_items?.length > 0 
                      ? (match as any).donating_items.join(", ") 
                      : (match.available_organs && match.available_organs.length > 0) 
                        ? match.available_organs.join(", ") 
                        : (match as any).organ_type || 'Specified Organ',
          blood_type: match.blood_type,
          match_score: match.ai_score,
          ai_explanation: match.ai_explanation,
          score_breakdown: (match as any).score_breakdown,
          distance_info: (match as any).travel_time_human || `${Math.round(match.distance_km * 10) / 10} km`,
          urgency: urgency
        })
      });
      ...
      toast.success("Emergency Alerts Dispatched!", {
        description: `Donor ${match.donor_name} has been notified via Email & SMS.`,
        duration: 5000
      });
      onClose();
    } catch (e: any) {
      toast.error(e.message || "Network synchronization failed.");
    } finally {
