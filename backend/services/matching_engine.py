...
class AIMatchingModel:
    ...
    async def predict_match_probability_async(self, donor: dict, recipient_lat: float, recipient_lon: float, use_road: bool = True) -> tuple[float, float]:
        """
        Runs the ML Inference asynchronously. 
        Returns (Match Percentage, Distance in km)
        """
        if use_road:
            distance, _ = await get_road_distance(recipient_lat, recipient_lon, donor.get('latitude', 0.0), donor.get('longitude', 0.0))
        else:
            distance = haversine(
                recipient_lat, recipient_lon, 
                donor.get('latitude', 0.0), donor.get('longitude', 0.0)
        ...
        if self.model: # Use Trained AI Model
            ...
            # Override distance in heuristic for consistent probability calculation if using road
            if use_road:
                dist_score = max(0, 1 - (distance / 500.0))
                heuristic['features'][1] = dist_score
    ...
    def predict_match_probability(self, donor: dict, recipient_lat: float, recipient_lon: float) -> tuple[float, float]:
        """
        Synchronous fallback (still uses Haversine). 
        Use predict_match_probability_async for road distance.
        """
        heuristic = self._extract_heuristic_features(donor, recipient_lat, recipient_lon)
